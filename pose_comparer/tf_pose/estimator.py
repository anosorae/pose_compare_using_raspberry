...
def compare(frame_vector, template_vector):
    return np.dot(frame_vector, template_vector) if frame_vector is not None and template_vector is not None else None
...
class TfPoseEstimator:
    ...
    def draw_humans(npimg, humans, imgcopy=False):
        ...
        for human in humans:
            ...
            for i in range(common.CocoPart.Background.value):
                # 如果COCO定义中某个关键点不在estimator得到的人体关键点中，开始下一次循环
    ...
    @staticmethod
    def draw_compared_human(npimg, humans, humans_template):
        # 图像的高度和宽度
        image_h, image_w = npimg.shape[:2]
        centers = {}
        centers_template = {}
        # 画出human和template的关节点和关节线
        for human, human_template in zip(humans, humans_template):
            # draw point
            i = 1
            while i <= common.CocoPart.Background.value:
                # 如果此关节点在human中或template_human中没有识别出来，跳过此次循环
                if i not in human.body_parts.keys() or i not in human_template.body_parts.keys():
                    i += 1
                    continue

                body_part = human.body_parts[i]
                body_part_template = human_template.body_parts[i]
                center = (int(body_part.x * image_w + 0.5), int(body_part.y * image_h + 0.5))
                center_template = (int(body_part_template.x * image_w + 0.5), int(body_part_template.y * image_h + 0.5))
                centers[i] = center
                centers_template[i] = center_template
                # 只画出human的点
                cv2.circle(npimg, center, 3, common.CocoColors[i], thickness=3, lineType=8, shift=0)
                # cv2.circle(npimg, center_template, 3, common.CocoColors[i], thickness=3, lineType=8, shift=0)
                i += 1

            # draw line
            for pair_order, pair in enumerate(common.CocoPairsRender):
                # 如果两个关节连线中的某个点没有识别出来，跳过此次
                if (pair[0] not in human.body_parts.keys() or pair[1] not in human.body_parts.keys()) and (pair[0] not in human_template.body_parts.keys() or pair[1] not in human_template.body_parts.keys()):
                    continue

                if centers.get(pair[0]) is None or centers.get(pair[1]) is None:
                    frame_vector = None
                else:
                    frame_vector = np.array(centers.get(pair[0]))-np.array(centers.get(pair[1]))
                    frame_vector = frame_vector / np.linalg.norm(frame_vector, axis=0)
                if centers_template.get(pair[0]) is None or centers_template.get(pair[1]) is None:
                    template_vector = None
                else:
                    template_vector = np.array(centers_template.get(pair[0])) - np.array(centers_template.get(pair[1]))
                    template_vector = template_vector / np.linalg.norm(template_vector, axis=0)
                compare_result = compare(frame_vector, template_vector)
                # print(frame_vector)
                # print(template_vector)
                if compare_result:
                    print(compare_result)
                    if compare_result < 0.95:
                        color = (0, 0, 255)
                    else:
                        color = (0, 255, 0)
                    cv2.line(npimg, centers.get(pair[0]), centers.get(pair[1]), color, 3)
